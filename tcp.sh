...
github1="raw.githubusercontent.com/padavan01"
...
startlotserver(){
	...
	bash <(wget --no-check-certificate -qO- https://${github1}/lotServer/master/Install.sh) install
...
remove_all(){
	...
	if [[ -e /appex/bin/lotServer.sh ]]; then
		bash <(wget --no-check-certificate -qO- https://${github1}/lotServer/master/Install.sh) uninstall
	fi
	clear
	echo -e "${Info}:清除加速完成。"
	sleep 1s
